    ...
    public partial class BuyVouchar : Form
    {
        ...
        {  //this try block for the vendor name and customer name ----------------------------------------
            ...
            {  
                ...
                OleDbCommand cmd = new OleDbCommand("Select v_id from BUY", ar.gcon);
                OleDbDataReader reader;
                ar.gcon.Open();
                reader = cmd.ExecuteReader();
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            {   OleDbCommand namecheck = new OleDbCommand($"Select full_name from ACCOUNTS where full_name = '{cb_vendor.Text}'", ar.gcon);
                string dr2;
                ar.gcon.Open();
                try { dr2 = Convert.ToString(namecheck.ExecuteScalar()); }
                catch { MessageBox.Show("Record Name Chaking Field Error:2"); dr2 = ""; }
                if (dr2 != cb_vendor.Text || cb_vendor.Text == "") { MessageBox.Show("Please Enter A valide name");cb_vendor.Focus(); save = false; }
            }
            ...
            if (save == true)
            {
                for (int i = records; i > 0; i--)
                {
                    ...
                    string challan = tb_challan.Text;
                    string vendor = cb_vendor.Text;
                    string customer = cb_cutomer.Text;
                    string trucknoo = tb_truck_no.Text;
                    string truckagc = tb_truck_agc.Text;
                    double bid = Convert.ToDouble(vouchar);
                    string item = "Not Asing";
                    int quantity = 0;
                    double rate = 0D;
                    double price = 0D;
                    //get the product details From product Controller -----------------------------------------
                    if (i == 6) { bid += 0.6; item = product6.Item; quantity = Convert.ToInt32(product6.Quantity); rate = Convert.ToDouble(product6.Rate); price = Convert.ToDouble(product6.Price); }
                    else if (i == 5) { bid += 0.5; item = product5.Item; quantity = Convert.ToInt32(product5.Quantity); rate = Convert.ToDouble(product5.Rate); price = Convert.ToDouble(product5.Price); }
                    else if (i == 4) { bid += 0.4; item = product4.Item; quantity = Convert.ToInt32(product4.Quantity); rate = Convert.ToDouble(product4.Rate); price = Convert.ToDouble(product4.Price); }
                    else if (i == 3) { bid += 0.3; item = product3.Item; quantity = Convert.ToInt32(product3.Quantity); rate = Convert.ToDouble(product3.Rate); price = Convert.ToDouble(product3.Price); }
                    else if (i == 2) { bid += 0.2; item = product2.Item; quantity = Convert.ToInt32(product2.Quantity); rate = Convert.ToDouble(product2.Rate); price = Convert.ToDouble(product2.Price); }
                    else if (i == 1) { bid += 0.1; item = product1.Item; quantity = Convert.ToInt32(product1.Quantity); rate = Convert.ToDouble(product1.Rate); price = Convert.ToDouble(product1.Price); }
                    else { MessageBox.Show("Error Ocure In get Product Details From Product Controler"); }
                    //submit data to Database ----------------------------------------------------------------------
                    OleDbCommand cmd = new OleDbCommand($"insert into SELLS values({vouchar},{bid},'{date}','{vendor}','{customer}','{trucknoo}','{truckagc}','{item}',{quantity},{rate},{price})", ar.gcon);
                ...
                ClearForm();
                if (IsCloseatSave) { BuyVouchar.ActiveForm.Close(); }
